...
function getTicketAll() {
    ...
    try {
        ...
        // Requête pour récupérer les données
        $query = "SELECT id, title, msg, created, ticket_status FROM tickets"; // Remplacez 'tickets' par votre table
        $stmt = $pdo->query($query);
        ...
        if ($stmt->rowCount() > 0) {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                echo   "<td><a href='gerer-ticket?id=" . $row['id'] . "'class='btn-create-ticket'>Gérer</a></td>
                    </tr>";
            }
...
function gererTicket() {
        ...
        if ($stmt = $conn->prepare($sql)) {
            ...
            if ($stmt->execute()) {
                ...
                header("Location: gerer-ticket?id=" . $ticket_id . "&success=true"); 
                exit();
            } else {
